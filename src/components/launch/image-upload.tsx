...
export function ImageUpload({launchId}:{launchId:string}){
  ...
  async function upload(file:File){
    ...
    await fetch("/api/uploads/confirm",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({launchId,key:signed.key,url:signed.publicUrl,fileName:file.name,mimeType:file.type,size:file.size})});
